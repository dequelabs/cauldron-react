...
test('__utils/AriaIsolate__', t => {
  t.test('AriaIsolate#activate', t => {
    t.test(
      'properly isolates the target by applying aria-hidden="true" to expected nodes',
      t => {
        t.plan(4);
        isolator.activate();
        ...
        t.equal(
          isolator.affectedElements.indexOf(
            document.getElementById('isolate-me')
          ),
          -1
        );
        t.equal(
          isolator.affectedElements.indexOf(
            document.getElementById('already-hidden')
          ),
          -1
        );
        // cleanup
  ...
  t.test('AriaIsolate#deactivate', t => {
    t.test(
      'properly removes aria-hidden from each of the affectedElements',
      t => {
        t.plan(2);
