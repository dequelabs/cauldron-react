...
test('__commons/Scrim__', t => {
  t.test('given a initial truthy `show` prop, fades scrim in', t => {
    ...
    setTimeout(() => { // wait for setState to complete
      t.equal(scrim.state('animationClass'), 'dqpl-scrim-show dqpl-scrim-fade-in');
    });
  });

  t.test('given a `show` prop update from falsey to truthy, calls fadeIn', t => {
    t.plan(3);
  ...
  t.test('given a `show` props update from truthy to falsey, calls fadeOut', t => {
    t.plan(2);
