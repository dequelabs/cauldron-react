...
test('__Alert Composite__', t => {
  ...
  t.test('focuses content when passed a truthy "show" prop upon mounting', t => {
    t.plan(1);
    const alert = mount(
      <Alert show={true}>{'hello'}</Alert>
    );
  ...
  t.test('focuses content when "show" prop is updated from falsey to truthy', t => {
    t.plan(1);
    const alert = mount(
      <Alert show={false}>{'hello'}</Alert>
    );
