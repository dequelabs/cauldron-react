...
test('renders the expected UI', () => {
  ...
  expect(wrapper.find('.dqpl-combobox')).toBeTruthy();
  expect(wrapper.find('.dqpl-listbox')).toBeTruthy();
});

test('clicking the label, focuses the combobox', () => {
  const wrapper = withDefaultSelected();
...
test('handles initially selected option', () => {
  ...
  expect(select.find('.dqpl-pseudo-value').text()).toBe('Bill');
});
...
test('clicking the combobox toggles the "expanded" state', () => {
...
test('handles "space" key (with collapsed list)', () => {
  const wrapper = basicSelect();
  expect(wrapper.state('expanded')).toBeFalsy();
